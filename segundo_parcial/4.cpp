...
void evolucion(LatticeBoltzmann &Ondas, std::ofstream &file){
  int t,tmax=80000;
  double rho0=0,Jx0=0,Jy0=0;
  std::vector<double> pmin(Lx-2,0.), pmax(Lx-2,0.);

  std::clog << "Constante de atenuación: " << D << std::endl;
  ...
  std::clog << "Standing Wave Ratio: " << SWR << std::endl
            << "Coeficiente de absorcion: " << C_abs << std::endl <<std::endl;
  file << D << "\t"<< C_abs << std::endl;
}

int main(){
  LatticeBoltzmann Ondas;

  std::ofstream file("data/4.dat");
  for(D=0.1;D <=0.9; D+=0.1){
    evolucion(Ondas, file);
  }
  file.close();
