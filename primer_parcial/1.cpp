...
void Colisionador::Force_betw(Cuerpo & Planeta1, Cuerpo & Planeta2){
	//No hay fuerzas entre cuerpos en este sistema
}
...
void InicieAnimacion(void){
  ...
  std::cout<<"set output 'data/1a.gif'"<<std::endl;
  std::cout<<"unset key"<<std::endl;
  std::cout<<"set xrange[0:16]"<<std::endl;
...
int main(){
	...
	double m=1, R=3;
	double x0=10;
	double kT=0.5, V0=sqrt(2*kT/m);
	double t,tmax=100, dt=0.01;
	double tdibujo,tcuadro=tmax/500;
	int i;

	planeta[0].Init(x0,0,0,V0,0,0,m,R);

	std::ofstream trayectoria("data/1b.dat");
	InicieAnimacion();
	for(t=0,tdibujo=0; t<tmax; t+=dt,tdibujo+=dt){
		...
		//Dibujar trayectoria
		trayectoria << t <<"\t"<< planeta[0].Getx() << std::endl;
