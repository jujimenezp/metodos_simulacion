...
void Colisionador::CalculeFuerzas(Cuerpo * Molecula){
  ...
  for(i=0;i<N;i++){
    ...
    if(s_Lx>0) F0.load(-K*pow(s_Lx,1.5),0,0); Molecula[i].AdicioneFuerza(F0);
    if(s_x0>0) F0.load(K*pow(s_x0,1.5),0,0); Molecula[i].AdicioneFuerza(F0);
    if(s_Ly>0) F0.load(0,-K*pow(s_Ly,1.5),0); Molecula[i].AdicioneFuerza(F0);
    if(s_y0>0) F0.load(0,K*pow(s_y0,1.5),0); Molecula[i].AdicioneFuerza(F0);
  }
...
int main(void){
  ...
  Crandom ran64(1);
  double m=1, R0=2.5, kT=2, V0=sqrt(2*kT/m);
  int i;
  double t,tdibujo,tf=200,tcuadro=tf/5000,dt=1e-3;
  double dx=10, dy=10;
